...
add_action('admin_enqueue_scripts', function($hook){
    ...
    if (!$page || strpos($page, 'product-launch-') !== 0) return; // only phase pages

    // Phase map (slug => human title). Keep in sync with submenu definitions.
    $pl_phase_map = array(
        'product-launch-market'  => __('Getting Market Clarity','product-launch'),
        'product-launch-offer'   => __('Creating Your Offer','product-launch'),
        'product-launch-service' => __('Creating Your Service','product-launch'),
        'product-launch-funnel'  => __('Building Sales Funnel','product-launch'),
        'product-launch-emailss'   => __('Writing Email Sequences','product-launch'),
        'product-launch-organic' => __('Organic Promo Posts','product-launch'),
        'product-launch-ads'     => __('Creating Facebook Ads','product-launch'),
        'product-launch-launch'  => __('Launching','product-launch'),
    );
    $order = array_keys($pl_phase_map);

    wp_register_script('pl-back-nav', '', array('jquery'), PL_PLUGIN_VERSION, true);
    wp_enqueue_script('pl-back-nav');
    wp_add_inline_script('pl-back-nav', '(function(){
        const params = new URLSearchParams(window.location.search);
        const current = params.get("page");
        if (!current || !/^product-launch-/.test(current)) return;

        const phaseMap = ' . json_encode($pl_phase_map) . ';
        const order    = ' . json_encode($order) . ';
        const idx = order.indexOf(current);
        const prevSlug = idx > 0 ? order[idx-1] : "product-launch"; // dashboard fallback
        const prevTitle = idx > 0 ? phaseMap[prevSlug] : "Dashboard";

        function makeBtn(){
            const a = document.createElement("a");
            a.href = "admin.php?page=" + prevSlug;
            a.className = "button button-secondary button-large pl-back-button";
            a.innerHTML = \'<span class="dashicons dashicons-arrow-left-alt"></span> \' +
                          ' . json_encode(__('Back to','product-launch')) . ' + " " + prevTitle;
            return a;
        }

        function inject(){
            const targets = document.querySelectorAll(".form-actions");
            if (targets.length) {
                targets.forEach(function(t){
                    // Avoid duplicates
                    if (t.querySelector(".pl-back-button")) return;
                    const btn = makeBtn();
                    // Prefer putting the back button at the start
                    t.insertBefore(btn, t.firstElementChild);
                });
                return true;
            }
            // Fallback: put at top of main phase container if form-actions not present
            const wrap = document.querySelector(".product-launch-phase-container .phase-header") ||
                         document.querySelector(".product-launch-phase-container") ||
                         document.querySelector(".wrap");
            if (wrap && !wrap.querySelector(".pl-back-button")) {
                const btn = makeBtn();
    ...
    // Minimal CSS for spacing (keeps WP styles)
    wp_register_style('pl-back-nav-style', false, array(), PL_PLUGIN_VERSION);
    wp_enqueue_style('pl-back-nav-style');
    wp_add_inline_style('pl-back-nav-style', '.pl-back-button{margin-right:10px;margin-bottom:10px;}');
});
// == /IS: Back Navigation Injection ==
